...
/**
 * FixLink business dashboard — Stage 7A (`/business`, authenticated
 * BUSINESS_OWNER / BUSINESS_MANAGER).
 *
 * Shows the server-derived business profile (name, status, technician
 * count) with an owner-only profile editor. Job management arrives in
 * a later stage: the jobs card is an explicit empty state, never fake
 * counts. Authorization is backend-enforced; the role checks here only
